    ...
    async (supabaseUser: SupabaseUser) => {
      try {
        ...
        const { data, error } = await supabase
          .from("user")
          .select(
            "first_name, last_name, account_type, created_at, account_balance"
          )
        ...
        if (error) {
          console.error(
            "[AuthProvider] Error fetching user profile from DB:",
            error,
            "for email:",
            supabaseUser.email
          );
        ...
        if (data) {
          ...
          console.error(
            "[AuthProvider] No user data found in DB for email:",
            supabaseUser.email
          );
          setUser(null);
        }
